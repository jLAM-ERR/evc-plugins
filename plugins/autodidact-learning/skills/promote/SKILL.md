---
name: promote
description: Promote a project knowledge entry that generalizes across projects into the shared autodidact knowledge base via PR, with the stricter destination secret gate and a redaction checklist. Run when the user asks to promote, or when distill flagged promote-eligible entries.
invocation: user
disable-model-invocation: true
---

# promote — project KB → shared autodidact KB
...
1. **Generalize.** Rewrite the entry body without project specifics; keep
   it ~10–30 lines. The autodidact entry is a NEW entry (fresh id from the new
   body) targeting the same category under the hub's `knowledge/`.
2. **Destination secret gate (stricter, per CONTRACT):** scan with the
   `hub` profile — ONLY the hub's `tools/allowlist.txt` applies; the project's
   local allowlist does NOT travel upstream:

   ```sh
   python3 <autodidact-checkout>/tools/kb_lint.py --root <autodidact-checkout> --layout hub
   ```
...
4. **Branch in the autodidact checkout**, add the entry file + its INDEX.md line
   (use the capture CLI with `--kb-root <autodidact-checkout>/knowledge` — never
...
6. Stop after opening: **a human reviews and merges** (CONTRACT
   §Moderation). the hub's own gardening dedupes arrivals from many projects.
