...
This repo is the autodidact execution layer: a Claude Code plugin marketplace
...
normative contract live in the sibling `autodidact` repo — `CONTRACT.md` there
governs every format, routing rule, and CLI protocol implemented here.

## Hard rules

- Every vendored `kblib` location here (`tools/kblib/`,
  `plugins/autodidact-learning/lib/`) is **copied from autodidact, never edited here** —
  fix in autodidact, re-copy *every* location, update each `SOURCE` marker. Real
  ...
  delete it. Rationale: autodidact `README.md` / `AGENTS.md`.
...
- Exit codes and JSON shapes of `new_entry.py` / `mechanical.py` are
  CONTRACT-frozen — changing them is a contract change, made in autodidact first
...
- Releases are annotated tags: `<plugin>-vX.Y.Z` matching that plugin's
  ...
  the two plugins version independently of each other and of autodidact.
